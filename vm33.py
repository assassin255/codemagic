...
# ============================
# BUILD QEMU 10.1.2 WITH PGO + BOLT + POLLY + FULL LTO + MAX TCG (TB=8GB)
# ============================
...
if choice == "y":
    if subprocess.run("command -v qemu-system-x86_64", shell=True).returncode == 0:
        ...
        run("sudo apt update -y && sudo apt install -y build-essential clang-15 lld-15 git ninja-build python3-venv python3-pip "
            ...
            "libsdl2-dev libsdl2-image-dev libspice-server-dev libspice-protocol-dev llvm-15 llvm-15-dev llvm-15-tools aria2")
        ...
        # ===== Env flags với Polly + vectorization + FULL LTO + TB=8GB =====
        env_base = (
            "export CC=clang-15; "
            "export CXX=clang++-15; "
            "export LD=lld-15; "
            "export COMMON='-O3 -march=native -mtune=native -pipe -flto -funroll-loops "
            "-fomit-frame-pointer -fno-semantic-interposition -mllvm -polly "
            "-mllvm -polly-vectorizer=stripmine'; "
        )

        # ===== STAGE 1: generate profile =====
        os.makedirs("/tmp/qemu-pgo-data", exist_ok=True)
        run(env_base + 
            'export CFLAGS="$COMMON -fprofile-generate=/tmp/qemu-pgo-data -DDEFAULT_TCG_TB_SIZE=8388608 -DTCG_TARGET_HAS_MEMORY_BARRIER=0"; '
            ...
            "--disable-werror --disable-debug-info --disable-malloc-trim --disable-checks")
        run("make -j$(nproc)")
        run("sudo make install DESTDIR=/tmp/qemu-pgo-install || sudo make install")

        # ===== STAGE 2: run workload để sinh .profraw =====
        ...
        if os.path.isdir(profdir):
            ...
            if profraws:
                ...
                print("⚠ Không có file .profraw để merge.")
        else:
            print("⚠ Thư mục profile không tồn tại, bỏ qua PGO merge.")

        # ===== STAGE 3: rebuild với profile =====
        os.chdir("/tmp/qemu-src/build")
        run(env_base + 
            'export CFLAGS="$COMMON -fprofile-use=/tmp/qemu_pgo.profdata -fprofile-correction -DDEFAULT_TCG_TB_SIZE=8388608 -DTCG_TARGET_HAS_MEMORY_BARRIER=0"; '
            'export CXXFLAGS="$CFLAGS"; '
            "export LDFLAGS='-flto -Wl,-O3'; "
            "make -j$(nproc) clean; "
            "../configure --target-list=x86_64-softmmu --enable-tcg --enable-slirp --enable-gtk "
...
-smp {cpu_core} \
-m {ram_size}G \
-accel tcg,thread=multi,tb-size=8388608,split-wx=off \
