...
choice = ask("👉 Build QEMU 10.1.2 tối đa hiệu năng (LTO + PGO + BOLT + POLLY)? (y/n): ", "n")
...
if choice == "y":
    ...
    run(env_base +
        ...
        "export LDFLAGS='-flto -Wl,-O3 -Wl,--gc-sections -Wl,--as-needed'; "
    ...
    run(env_base +
        ...
        "-DDEFAULT_TCG_TB_SIZE=16384 -DTCG_TARGET_HAS_MEMORY_BARRIER=0\"; "
        "export CXXFLAGS=\"$CFLAGS\"; "
        "export LDFLAGS='-flto -Wl,-O3 -Wl,--gc-sections -Wl,--icf=all -Wl,--as-needed'; "
        "make clean; "
        "../configure --target-list=x86_64-softmmu --enable-tcg --enable-slirp "
        "--enable-gtk --enable-sdl --enable-spice --enable-plugins --enable-lto "
        "--enable-coroutine-pool --disable-werror --disable-strip --disable-docs "
