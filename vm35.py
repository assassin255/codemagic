...
if choice == "y":
    ...
    os.environ["PATH"] = "/usr/lib/llvm-15/bin:" + os.environ.get("PATH", "")
    ...
    # COMMON optimization flags (no LLVM-full-only flags)
    common_flags = (
        "-O3 -march=native -mtune=native -pipe -flto "
    ...
    # Ensure we use the just-built qemu to generate profiles
    os.environ["PATH"] = "/tmp/qemu-pgo-install/usr/local/bin:" + os.environ.get("PATH", "")

    # Run small workload to emit .profraw files (adjust workload to your real tests)
    print("🔥 Running quick workload to generate .profraw files...")
    for cmd in ["qemu-system-x86_64 --version", "qemu-img --version", "qemu-system-x86_64 -h | head -n 5"]:
        run(cmd)

    profdir = "/tmp/qemu-pgo-data"
    if os.path.isdir(profdir):
        profraws = " ".join(
            os.path.join(profdir, f) for f in os.listdir(profdir) if f.endswith(".profraw")
        )
        if profraws:
            run(f"llvm-profdata merge -output=/tmp/qemu_pgo.profdata {profraws}")
        else:
            print("⚠ Không tìm thấy .profraw — bạn cần chạy workload đủ lâu để tạo profile.")
    else:
        print("⚠ Thư mục profile /tmp/qemu-pgo-data không tồn tại — bỏ qua merge.")

    # PHASE 2: Final LTO + PGO build (if profile exists)
    os.chdir("/tmp/qemu-src/build")
    run(env_base +
        "export CFLAGS=\"$COMMON -fprofile-use=/tmp/qemu_pgo.profdata -fprofile-correction "
        "-DDEFAULT_TCG_TB_SIZE=16384 -DTCG_TARGET_HAS_MEMORY_BARRIER=0\"; "
        "export CXXFLAGS=\"$CFLAGS\"; "
        "export LDFLAGS='-flto -Wl,-O3 -Wl,--gc-sections -Wl,--icf=all -Wl,--as-needed -Wl,--icf=all'; "
        "make clean; "
        "../configure --target-list=x86_64-softmmu --enable-tcg --enable-slirp "
        "--enable-gtk --enable-sdl --enable-spice --enable-plugins --enable-lto "
        "--enable-coroutine-pool --disable-werror --disable-strip --disable-docs "
    ...
    # BOLT post-link with requested extra passes
    ...
    if os.path.exists(qemu_bin) and subprocess.run("command -v llvm-bolt", shell=True).returncode == 0:
        ...
        print("⚡ llvm-bolt không có sẵn — bỏ qua bước BOLT.")
...
# runtime TCG tuned: increase tb-size, disable split-wx for better JIT locality
