...
# ===== Detect Ubuntu & choose LLVM =====
llvm_pkg = ""
llvm_bin_path = ""
try:
    ubuntu_ver = subprocess.getoutput("lsb_release -rs")
except:
    ubuntu_ver = ""

if ubuntu_ver.startswith("20") or ubuntu_ver.startswith("22"):
    print("✅ Ubuntu <=22 detected, using LLVM 15")
    llvm_pkg = "clang-15 lld-15 llvm-15 llvm-15-dev llvm-15-tools"
    llvm_bin_path = "/usr/lib/llvm-15/bin"
else:
    print("⚠ Ubuntu >=23 detected, using LLVM 20")
    llvm_pkg = "clang-20 lld-20 llvm-20 llvm-20-dev llvm-20-tools"
    llvm_bin_path = "/usr/lib/llvm-20/bin"

# ===== Build QEMU =====
print("\n========== Build QEMU ==========")
choice = ask("👉 Bạn có muốn build QEMU 10.1.2 với TCG+Polly+LTO + fast-math không? (y/n): ", "n")

if choice == "y":
    run(f"sudo apt update -y && sudo apt install -y build-essential git ninja-build python3-venv python3-pip "
        f"libglib2.0-dev libpixman-1-dev zlib1g-dev libfdt-dev libslirp-dev libusb-1.0-0-dev "
        f"libgtk-3-dev libsdl2-dev libsdl2-image-dev libspice-server-dev libspice-protocol-dev "
        f"{llvm_pkg} aria2")

    os.environ["PATH"] = llvm_bin_path + ":" + os.environ["PATH"]
    ...
    env_base = (
        f"export CC={llvm_bin_path}/clang; "
        f"export CXX={llvm_bin_path}/clang++; "
        f"export LD={llvm_bin_path}/lld; "
        "export COMMON='-Ofast -ffast-math -funroll-loops -fomit-frame-pointer "
        "-flto -fno-semantic-interposition -fno-exceptions -fno-rtti -fno-asynchronous-unwind-tables "
    ...
    run(
        env_base +
        " export CFLAGS=\"$COMMON -fno-pie -fno-pic -DDEFAULT_TCG_TB_SIZE=32768 -DTCG_TARGET_HAS_MEMORY_BARRIER=0\"; "
        ...
        "--disable-malloc-trim "
        "--disable-plugins "
        "--extra-cflags='-DDEFAULT_TCG_TB_SIZE=32768 -DTCG_TARGET_HAS_MEMORY_BARRIER=0'"
    )
    ...
    print("✅ QEMU 10.1.2 built với TCG+Polly+LTO + fast-math, bỏ qua FENV access!")
else:
